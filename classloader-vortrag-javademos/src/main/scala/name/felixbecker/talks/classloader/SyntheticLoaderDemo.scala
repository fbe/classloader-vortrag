...
class SyntheticClassLoader extends ClassLoader {

  def greeterClassBytes: Array[Char] = Array[Char](
  ...
  def createSyntheticGreeterClass: Class[_] = {
    defineClass("name.felixbecker.talks.classloader.GreeterClass", charToByteArray(greeterClassBytes), 0, greeterClassBytes.length)
  }

  private def charToByteArray(chars: Array[Char]): Array[Byte] = {
     chars.map(_.asInstanceOf[Byte])
  }
}

object SyntheticLoaderDemo extends App {
  val cl = new SyntheticClassLoader
